...
const UsersRejectedTaskSolutions = () => {
  const { id } = useParams();
  const [taskSolutions, setTaskSolutions] = useState([]);
  const [errorMessage, setErrorMessage] = useState("");

  useEffect(() => {
    fetchUsersRejectTaskSolutions();
  }, [id]);

  const fetchUsersRejectTaskSolutions = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:3000/api/rejected-task-solution/${id}`,
        {
          headers: {
            token: token,
          },
...
export default UsersRejectedTaskSolutions;
